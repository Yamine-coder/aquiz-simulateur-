...
export interface AnnonceScoreData extends ScoreComparateurResult {
  enrichissement?: {
    ...
    quartier?: { success: boolean; scoreQuartier?: number; transports?: number; commerces?: number; ecoles?: number; sante?: number; espaceVerts?: number; transportsProches?: Array<{ type: string; typeTransport: string; nom: string; distance: number; lat?: number; lon?: number; lignes?: string[]; operateur?: string; couleur?: string }>; transportSummary?: Array<{ type: string; lignes: string[]; count: number; nearestWalkMin?: number }>; counts?: { transport: number; commerce: number; education: number; sante: number; loisirs: number; vert: number }; detailedCounts?: Record<string, Array<{ type: string; label: string; count: number }>> }
